package com.atguigu.sparkmall0705.mock
...
object MockerOffline {
  ...
  def mockUserAction() = {
    ...
    for (i <- 1 to sessionNum) {
      ...
      val sessionId = UUID.randomUUID().toString  //随机码
      var isQuit = false

      while (!isQuit) {
        val action = actionsOptions.getRandomOpt() //随机产生一个行为

        if (action == "quit") {  //如果是quit session结束
          isQuit = true
        } else {
          ...
          action match {
            ...
            case "order" => orderCategoryIds = RandomNum.multi(1, cargoryNum, RandomNum(1, 5), ",", false) //5,11,12,18 //1,4 //2  //1,1,2
